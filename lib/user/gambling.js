exports.GTN = (database, api, parameters, message) => {
    ...
    let answer = Math.ceil(Math.random() * (maxAnswer - 0 + 1)) + 0;

    if(coins < 5) {
        api.say("You must be willing to lose at least 5 coins to play GTN.");
        return;
    }

    database.users.findById(message.userId).then(user => {
        if(user.balance < coins) {
            api.say(message.username + "...you do not have enough coins to play GTN.");
            return;
        }
    });

    if(returnPercent < 15) {
        api.say(message.username + ", you need a return percent of at least 15% to play GTN.");
        return;
    }
    ...
    if(guess == answer) {
        ...
        database.users.findById(message.userId).then(user => {
            user.balance = Number(user.balance + newCoins);
            user.save().then(() => {
